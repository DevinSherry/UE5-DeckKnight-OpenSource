...
TWeakObjectPtr<APawn> FGASCResourceModifcationEventsPanel::SelectedPawn = nullptr;

EGASC_ResourcePipelineType FGASCResourceModifcationEventsPanel::SelectedPipelineType = Healing;
TMap<uint32, bool> FGASCResourceModifcationEventsPanel::DrawHitResultStates;
TMap<uint32, ImVec4> FGASCResourceModifcationEventsPanel::DrawHitResultColors;
TMap<uint32, uint32> FGASCResourceModifcationEventsPanel::DrawHitResultRadii;
bool FGASCResourceModifcationEventsPanel::bFilterOutSimulated = false;
int FGASCResourceModifcationEventsPanel::DamageModContextFilter = 0;
int FGASCResourceModifcationEventsPanel::DamageModEventType = 0;
int FGASCResourceModifcationEventsPanel::DamageType = 0;

bool FGASCResourceModifcationEventsPanel::bImmuneDamageAll = false;
bool FGASCResourceModifcationEventsPanel::bImmuneDamageFire = false;

FGASCResourceModifcationEventsPanel::FGASCResourceModifcationEventsPanel()
{
...
void FGASCResourceModifcationEventsPanel::DrawDebugPanel(bool& bOpen)
{
    if (!bOpen)
        return;
        ...
        ImGuiTableFlags_BordersH))
    {
        ...
        // -------- Pipeline Type --------
        ImGui::TableNextColumn();
        FString EnumStr = StaticEnum<EGASC_ResourcePipelineType>()->GetNameStringByValue((int64)SelectedPipelineType);
        const char* AnsiName = TCHAR_TO_ANSI(*EnumStr);

        if (ImGui::BeginCombo("Modification Type", AnsiName))
        {
            UEnum* EnumPtr = StaticEnum<EGASC_ResourcePipelineType>();
            int32 EnumCount = EnumPtr->NumEnums() - 1;

            for (int32 i = 0; i < EnumCount; i++)
            {
                EGASC_ResourcePipelineType EnumValue = (EGASC_ResourcePipelineType)EnumPtr->GetValueByIndex(i);
    ...
    UResourcePipelineDebugSubsystem* DebugSys = SelectedPawn->GetWorld()->GetSubsystem<UResourcePipelineDebugSubsystem>();
    if (!DebugSys)
    {
        ImGui::End();
        return;
        ...
        ImGuiTableFlags_BordersH))
    {
        ...
        TArray<FResourceLogEntry> DamageEntries = DebugSys->GetResourceLogEntriesForActorID(SelectedPawn->GetUniqueID());

        for (FResourceLogEntry& Entry : DamageEntries)
        {
            if ((Entry.bIsDamageEffect && SelectedPipelineType != Damage) ||
                (!Entry.bIsDamageEffect && SelectedPipelineType != Healing))
                continue;

            AActor* Inst = DebugSys->GetActorFromID(Entry.ResourceInstigatorID);
            AActor* Target = DebugSys->GetActorFromID(Entry.ResourceTargetID);
            ...
            ImGui::TextColored(modColorV, "Base: %.3f", Entry.BaseResourceValue);
            ImGui::TextColored(modColorV, "Modified: %.3f", Entry.ModifiedResourceValue);
            ImGui::TextColored(modColorV, "Final: %.3f", Entry.FinalResourceValue);
            ...
            FString HRScrollID = FString::Printf(TEXT("HitResultData##%i"), Entry.ResourceID);
            ...
            if (Entry.HitResult.bBlockingHit)
            {
                FString HitOptId = FString::Printf(TEXT("HitResultOptions##%i"), Entry.ResourceID);
                    ...
                    ImGuiTableFlags_BordersH))
                {
                    ...
                    // Draw checkbox
                    ImGui::TableNextColumn();
                    bool& bDrawHit = DrawHitResultStates.FindOrAdd(Entry.ResourceID);
                    ImGui::Checkbox("##DrawHR", &bDrawHit);

                    // Color picker
                    ImGui::TableNextColumn();
                    ImVec4& HRColor = DrawHitResultColors.FindOrAdd(Entry.ResourceID, ImVec4(1, 0, 0, 1));
                    ...
                    // Radius slider
                    ImGui::TableNextColumn();
                    uint32& DebugRadius = DrawHitResultRadii.FindOrAdd(Entry.ResourceID, 20);
            ...
            ImGui::TableNextColumn();
            FString SimID = FString::Printf(TEXT("Simulate %s ID: %i"), *Cat, Entry.ResourceID);
            auto SimIDAnsi = StringCast<ANSICHAR>(*SimID);

            if (ImGui::Button(SimIDAnsi.Get()))
            {
                DebugSys->SimulateResourceModFromID(Entry.ResourceID);
            }
        }
        ImGui::EndTable();
    }
...
void FGASCResourceModifcationEventsPanel::UpdateCachedPawns(TArray<TWeakObjectPtr<APawn>> Pawns)
{
	CachedPawns = Pawns;
}
